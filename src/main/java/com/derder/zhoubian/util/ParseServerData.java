...
public class ParseServerData {
    ...
    public ParseServerData(InteractServer interactServer) {
    ...
    public List<EsItem> esList(String url,Long pullOffset,Integer limit) throws IOException, JSONException {
        ...
        Map<String,String> resultMap = interactServer.getDataFromServer(url+"/limit/"+limit+where);
        Log.d("esList","res.code===>["+resultMap.get(interactServer.getRESULT_MAP_CODE_KEY())+"]");
        Log.d("esList","res.message===>["+resultMap.get(interactServer.getRESULT_MAP_MESSAGE_KEY())+"]");
        if (resultMap.get(interactServer.getRESULT_MAP_CODE_KEY()).equals(HttpStatus.SC_OK+"")) {
            String res = resultMap.get(interactServer.getRESULT_MAP_MESSAGE_KEY());
